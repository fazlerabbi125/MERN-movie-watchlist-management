...
export function OAuthButtons({ onSuccess, onError }: OAuthButtonsProps) {
    ...
    const handleOAuth = async (provider: 'google' | 'discord') => {
        ...
        try {
            const authUrl = await AuthService.startOAuthFlow(provider);
    ...
    return (
        <div className="space-y-3">
            ...
            <button
                ...
                disabled={loading !== null}
                className="w-full flex justify-center items-center py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50"
            >
                {loading === 'discord' ? (
